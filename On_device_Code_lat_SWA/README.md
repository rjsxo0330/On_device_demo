...
This folder contains the lateral-only on-device SILS controller with steering wheel angle output.
...
On_device_main_sils.c
    ...
    -> Write Gateway CAN steering wheel angle output
```
...
|---|---:|---|---|
| `y0` | 1 | Gateway CAN | steering wheel angle command in rad |
...
`LaneKeepingController()` outputs `Logic_ref_SWA` directly.
